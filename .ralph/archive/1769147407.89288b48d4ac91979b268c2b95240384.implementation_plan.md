...
- Acceptance Criteria:
  ...
  - Subdirectories: `archive/`, `progress/`, `templates/`, `logs/`
...
- Status: Pending
- Description: Implement minimum iteration limit calculation (X^(Y+1)+5) with user confirmation
- Acceptance Criteria:
...
- Acceptance Criteria:
  - Templates stored in `.research/{name}/templates/`
...
- Acceptance Criteria:
  ...
  - --depth Y (default 3)
  - --name Z (required)
  - --max-iterations N (calculated minimum)
...
- Acceptance Criteria:
  ...
  - Progress files in subdirectories archived with subtopic prefix
...
5. **create_research_plan_management** depends on RWRState
6. **implement_progress_file_management** depends on research plan management
7. **implement_bfs_research_phase** depends on progress file management and template system
8. **implement_review_phase** depends on BFS research phase
9. **implement_synthesize_phase** depends on progress file management
10. **implement_final_review_revise_cycle** depends on synthesize phase
11. **implement_template_system** is independent
12. **implement_cli_parameters** depends on state management
13. **implement_archive_management** depends on directory structure and template system
14. **implement_completion_detection** depends on all phase implementations
15. **implement_error_handling_recovery** depends on phase implementations
16. **integrate_main_loop** depends on all other tasks
